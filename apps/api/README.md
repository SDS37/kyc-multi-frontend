...
| `case` | Authenticated (any role) | Detail by `id`; same visibility as `cases`; returns `case` (incl. `formData`), `comments` (from `reviewComment`), `documents` (metadata only; empty until KYC-040 — never file bytes) |
...
| KYC-037 | Authenticated `case(id)` detail; same visibility as list; FormData + comments; document metadata empty (no bytes) until KYC-040 |
