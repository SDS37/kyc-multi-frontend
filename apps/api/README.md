...
Local Development listens on **HTTP** (`http://localhost:5295`). That is acceptable for documented Compose credentials only — do not use plain HTTP for real secrets. HTTPS redirect and HSTS are **off** in Development. Browser apps on `http://localhost:4200` (Angular), `http://localhost:5173` (React), and `http://localhost:5174` (Vue), plus Vite preview `http://localhost:4173` (React) and `http://localhost:4174` (Vue), are on the CORS allow-list (`Cors:AllowedOrigins`). Every response sends `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, and `Content-Security-Policy: default-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'none'` (issue #108). That CSP applies to API responses as documents; it does not constrain the UIs. Opening `/graphql` in a browser (Banana Cake Pop) will not run scripts under that policy — use the UIs or a GraphQL client. What the UIs must allow is in [frontend-code-standards.md](../../docs/frontend-code-standards.md#content-security-policy-all-frontends). Outside Development the API also redirects HTTP → HTTPS, except `GET /health` and `GET /ready` (so probes can stay HTTP) and requests that already carry `X-Forwarded-Proto: https` (so a TLS-terminating proxy that is not in KnownProxies cannot loop).

**Production reverse proxy:** keep the default `ForwardedHeaders` `KnownProxies` / `KnownIPNetworks` (loopback) or **set them to your proxy**. Do not `Clear()` them in Production — that would trust `X-Forwarded-For` from anyone and move the rate-limit partition off the real client IP. Development clears those lists so local reverse proxies work. Committed `appsettings.Production.json` sets `AllowedHosts` to `localhost` (not `*`); override with the real public host (`AllowedHosts` / `AllowedHosts__0`).
...
| KYC-051 | Reviewer/TenantAdmin `caseAuditEntries(caseId)`; customers blocked; newest first; includes document uploads for the case |
...
| KYC-091 | CORS allow-list `http://localhost:4200`, `http://localhost:5173`, and `http://localhost:5174` (`Cors:AllowedOrigins`); preflight on GraphQL and document REST; basic headers + non-Dev HSTS |
| #108 | `Content-Security-Policy` on the API; HTTPS redirect outside Development; Vite preview origins `4173` / `4174` on CORS |
