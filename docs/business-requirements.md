...
**Status**: Aligned with accepted ADRs; W1–W5 delivered on `main` (API + Angular admin + React customer). Vue reports overview (KYC-080–081) is on `main`. KYC-091 (local CORS + basic headers) and KYC-093 (auth abuse controls) are done. Remaining Week 6 is KYC-094 (frontend 429 + optional login captcha), seed (KYC-101), runbook (KYC-100), and CSP/HTTPS ([#108](https://github.com/SDS37/kyc-multi-frontend/issues/108)).
