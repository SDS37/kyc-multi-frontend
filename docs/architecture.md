...
**Today on `main`:** Compose (Postgres / Redis / MinIO); .NET API with EF Core, Tenant/User, temporary public register/login (JWT), fail-closed `ICurrentTenant` + `ITenantScoped` EF filters; Hot Chocolate `/graphql` + `/health` (GraphQL IDE in Development); `apps/api/Kyc.Api.sln` + tests; GitHub Actions `api-ci` (KYC-102). Cases and UI apps are not built yet; GraphQL JWT auth is KYC-021.
