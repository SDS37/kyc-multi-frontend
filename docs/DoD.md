### Definition of Done for Project 1 MVP

- [x] Customer can create, fill, upload, submit — **React** (KYC-070–074, W5)
- [x] Reviewer can review, approve/reject — **Angular** (KYC-060–065, W4)
- [x] Tenant isolation tested (JWT + EF filters; Case inherits `ITenantScoped`)
- [ ] 3 frontends consume the same GraphQL API — Angular + React done; Vue **login/shell** (KYC-080); case overview still KYC-081
- [x] Architecture diagrams published (target vs today called out)
- [x] README allows a colleague to run the system locally (Compose + API + Angular + React + Vue runbooks; `api-ci` / `angular-ci` / `react-ci` / `vue-ci`)
