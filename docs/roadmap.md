...
- **KYC-110** — **Done.** Playwright Chromium smoke per frontend (`npm run test:e2e`, not `test:ci`). CI: Postgres → Development API + seed → serve the app. Path-filtered `angular-e2e` / `react-e2e` / `vue-e2e`.
