...
| **W6** | Next | Security + Seed | KYC-100, KYC-101, KYC-110, [KYC-095](https://github.com/SDS37/kyc-multi-frontend/issues/114) (**done**), [issue #108](https://github.com/SDS37/kyc-multi-frontend/issues/108) (CSP / HTTPS redirect — **done**) | Isolation tests green; demo seed; leave-localhost hardening |
| **W7** | Planned | Federation attempt, polish, docs | leftover + Module Federation spike | Public README and architecture complete |

**W5+ demoable now:** Customer signs in (React), creates a draft, fills FormData, uploads PDF/PNG/JPG (≤10 MB), and submits. Reviewer finishes the case in Angular (W4). Vue reports overview is KYC-081 (counts + latest 10). Remaining W6 is Playwright smoke.
