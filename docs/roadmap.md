...
| **W6** | Next | Security + Seed | KYC-094 (frontend 429 + optional login captcha), KYC-100, KYC-101, KYC-110, [issue #108](https://github.com/SDS37/kyc-multi-frontend/issues/108) (CSP / HTTPS redirect) | Isolation tests green; demo seed; leave-localhost hardening |
...
- **KYC-094** — Angular / React / Vue login: HTTP 429 → dedicated catalog copy (not a network failure; session stays); optional `captchaToken` on GraphQL `login` when `Captcha:RequiredForLogin` (Turnstile widget or test-provider field). Development unchanged.
